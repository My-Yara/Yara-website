...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { statusCode: 500, body: JSON.stringify({ message: 'Failed to save data to GitHub.', details: errorBody }) };
        }

        return { statusCode: 200, body: JSON.stringify({ message: 'Response saved and encrypted successfully!' }) };

    } catch (error) {
        console.error('Function execution error:', error);
        return { statusCode: 500, body: JSON.stringify({ message: 'Serverless function execution failed.' }) };
    }
};
